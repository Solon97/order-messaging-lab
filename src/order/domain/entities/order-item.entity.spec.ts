...
describe('OrderItem', () => {
  describe('create', () => {
    ...
    it('throws InvalidOrderItemError when sku is empty', () => {
      expect(() =>
        OrderItem.create({ sku: '', quantity: 1, unitPrice: 1 }),
    ...
    it('throws InvalidOrderItemError when quantity is less than or equal to zero', () => {
      expect(() =>
        OrderItem.create({ sku: 'SKU-1', quantity: 0, unitPrice: 1 }),
      ).toThrow(InvalidOrderItemError);
      expect(() =>
        OrderItem.create({ sku: 'SKU-1', quantity: -1, unitPrice: 1 }),
      ).toThrow(InvalidOrderItemError);
    });

    it('throws InvalidOrderItemError when unitPrice is negative', () => {
      expect(() =>
        OrderItem.create({ sku: 'SKU-1', quantity: 1, unitPrice: -0.01 }),
      ).toThrow(InvalidOrderItemError);
    });
